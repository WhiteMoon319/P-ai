...
群聊不能把所有消息都直接交给助理，否则助理会介入与自己无关的话题。每条消息都先入库；只有通过入场判断的消息，才会开始一个巡检批次。

```text
消息入库
→ 当前已有已入场批次：只追加消息，保持原巡检等级
→ 当前没有批次：判断入场模式
  - 始终入场：通过
  - 点名入场：命中点名词才通过
  - 不入场：不通过
...
未通过点名入场的消息只保留在会话中，不创建秘书巡检、不调用模型。巡检是已经入场、正在处理当前批次时才会发生的事情；联系人历史上的 `Present` 不能充当下一条消息的免检通行证。
...
2. 本批后续文字、图片、音频或附件只会追加到结束锚点。
3. 巡检等级和已排定的截止时间由入场消息固定；后续消息无需再次点名，也不会因再次点名而改写路径。
4. 锚点之前的所有群友消息仍按原始顺序进入正式连续上下文。
...
秘书巡检用于“刚通过入场、但入场消息没有点名”的情况。它不是未入场消息的兜底入口：人没有入场，就没有工作台可巡检。
...
1. 未点名的入场消息创建一次以 60 秒为基础、带随机波动的秘书巡检。
2. 本批后续消息只更新结束锚点，不重置已经排定的巡检时间，也不改变为点名巡检。
3. 巡检到期后，秘书统一判断这段话题是否与助理有关。
4. 秘书可以选择：
   - 不回复；
   - 创建新的远程应答委托；
...
同一联系人同一时刻只有一个已入场批次。入场消息决定该批次走助理防抖还是秘书巡检；后续消息只能追加，不能创建第二个批次，也不能通过再次点名升级或降级本批路径。
...
| 未点名处理 | 仍然直接调度当前会话 | 仅始终入场模式可启动 60 秒基础秘书巡检；未入场消息只保存 |
...
私聊没有秘书、闭嘴/张嘴和离场/在场，所有有效消息强制调度当前绑定会话，忙碌时统一作为引导消息加入；群聊则先把消息写入当前 block，再以入场模式决定是否创建批次：未入场只保存，已入场批次固定为点名助理防抖或非点名秘书巡检，后续消息只追加，不再改变巡检等级。
