...
2. 范围冻结前的普通消息不更新状态；点名消息升级为点名巡检并按点名防抖重新排定到期时间。
...
同一联系人同一时刻只有一个已入场批次。入场消息决定初始巡检等级；范围冻结前点名可以把非点名巡检升级为助理防抖，普通消息不能创建第二个批次或重置计时。冻结后的点名只记录下一轮等级。
...
私聊没有秘书、闭嘴/张嘴和离场/在场，所有有效消息先入库再调度当前绑定会话，忙碌时只把消息 ID 作为引导引用加入；群聊则先把消息写入当前 block，再以入场模式决定是否创建批次：未入场只保存，已入场批次按点名助理防抖或非点名秘书巡检处理，范围冻结前点名可升级，冻结后只影响下一轮。
