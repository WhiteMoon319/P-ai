...
- 修复 Android 远程前端模式三功能失效（通知转发、会话列表、新增对话）：安卓壳层桥接 origin 对齐上游 https（`tauri-api` 的 REMOTE_BRIDGE_ALLOWED_ORIGIN 改为 `https://tauri.localhost`），补齐 `tauri.android.conf.json` 的 `useHttpsScheme` 开关，并放开 WebView 混合内容以兼容壳层 https 加载电脑 iframe
- 修复 Windows 平台保护：AUMID 注册调用加 `#[cfg(windows)]` 守卫，避免 macOS/Linux 编译失败
