...
- 新增（android-live-update）：Android 后台任务保活前台服务：仅当应用处于后台且存在 ongoing + promoted 的 live 通知时启动 `specialUse` 前台服务（绑定最后一条活跃 live 通知，无额外通知条），提升进程优先级保证后台轮询与网络请求不被系统回收或 Doze 深度限制；应用回到前台或全部 live 通知结束后服务自动停止，通知保留由终态通知覆盖。插件 manifest 新增 `FOREGROUND_SERVICE` / `FOREGROUND_SERVICE_SPECIAL_USE` 权限与 service 声明。
