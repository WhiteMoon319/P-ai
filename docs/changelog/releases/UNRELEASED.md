...
- 修复模型显示名编辑不生效：模型卡内修改显示名后，脏检查（dirty）比对未感知该字段，保存/还原按钮保持禁用；同时保存载荷与启动加载路径也丢弃了显示名，导致即使保存成功重启后也会丢失。现已在脏检查、克隆、保存载荷、加载四条链路补齐 `displayName` 字段透传
- 模型显示名全链路生效：除模型设置卡外，配置页连通性测试下拉、模型下拉树（专用组件）、聊天窗口模型选择、部门人格模型名等读模型名的位置统一优先显示显示名（回退原模型名），apiConfigs 派生与持久化同步携带显示名
- 修复模型显示名只对单条生效：模型卡聚合同一模型的多个思维等级变体，此前编辑显示名只写入代表卡片，同组其他思维等级仍显示原始模型名；现显示名同步到组内全部变体，简单页保存/草稿/加载链路同步携带显示名
