...
- operate 截图总是保存并返回路径：脚本未给 `save=` 时也落盘到 `temp/screenshots/operate_{时间戳}.webp`（应用数据根目录下，重启不清），给了 `save=` 仍存指定路径；驱动模型不支持图片时不再拒绝截图，只返回保存路径（跳过 base64 编码），支持图片时额外携带 base64；会话上下文压缩时清理 `temp/screenshots/` 目录。
