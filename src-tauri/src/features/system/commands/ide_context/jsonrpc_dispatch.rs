...
async fn ide_chat_handle_jsonrpc_request(
    ...
    let result = match request.method.as_str() {
        ...
        "workspace.directory.list" => Err("Android 不支持工作区目录浏览".to_string()),
        "workspace.gitRootCheck" => Err("Android 不支持 Git 工作区检查".to_string()),
        ...
        "update_chat_shell_workspace_layout" => ide_chat_parse_workspace_params::<SaveChatShellWorkspacesInput>(request.params)
            ...
            .and_then(ide_chat_serialize),
        "check_git_workspace_root" => Err("Android 不支持 Git 工作区检查".to_string()),
        "fileReader.directory.list" => Err("Android 不支持文件读取器".to_string()),
        "fileReader.readFile" => Err("Android 不支持文件读取器".to_string()),
        "fileReader.readFileBlock" => Err("Android 不支持文件读取器".to_string()),
        ...
        "apply_import_config_migration_package" | "configMigration.apply" => {
            Err("Android 不支持应用迁移包（依赖桌面重启机制）".to_string())
        }
        "codex_get_auth_status" => {
            Err("Android 不支持 Codex 认证".to_string())
        }
        "codex_start_oauth_login" => {
            Err("Android 不支持 Codex OAuth 登录".to_string())
        }
        "codex_get_rate_limits" => {
            Err("Android 不支持 Codex 限流查询".to_string())
        }
        "codex_consume_rate_limit_reset_credit" => {
            Err("Android 不支持 Codex 限流重置".to_string())
        }
        "codex_logout" => Err("Android 不支持 Codex 认证".to_string()),
