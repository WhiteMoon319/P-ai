...
fn remote_im_group_reply_reconfigure_contact(
    ...
    store.by_contact.insert(
        ...
        RemoteImGroupReplyState {
            ...
            end_message_id: existing.end_message_id,
            decision_end_message_id: None,
            pending_start_message_id: None,
            focus: existing.focus || existing.pending_focus,
            pending_focus: false,
    ...
    let action = {
        ...
        match current {
            Some(current) if current.generation == generation => {
                if let Some(pending_start) = current.pending_start_message_id {
                    let next_generation = remote_im_group_reply_next_generation(&mut store);
                    store.by_contact.insert(
                        key.clone(),
                        RemoteImGroupReplyState {
                            generation: next_generation,
                            phase: RemoteImGroupReplyPhase::NonMentionScheduled,
                            start_message_id: pending_start,
                            end_message_id: current.end_message_id,
                            decision_end_message_id: None,
                            pending_start_message_id: None,
                            focus: current.pending_focus,
                            pending_focus: false,
                            event: current.event,
                            due_at: std::time::Instant::now() + delay,
                            inspection_kind: RemoteImGroupReplyTimerKind::NonMention,
                            pending_settlement: None,
                        },
                    );
                    Some(RemoteImGroupReplyTimerAction {
...
fn remote_im_group_reply_recovery_start_message_id(
    state: &AppState,
    contact_id: &str,
    conversation_id: &str,
    ...
    let runtime = match state_read_runtime_state_cached(state) {
        ...
        Err(err) => {
            runtime_log_warn(format!(
                "[群聊巡检] 恢复边界读取失败，contact_id={}，error={}",
                contact_id, err
            ));
            return current_message_id.to_string();
        }
    };
    ...
    let Some(boundary) = checkpoint
        .and_then(|item| {
            item.group_reply_delivery
                .as_ref()
                .filter(|marker| marker.status == "dispatching")
                .map(|marker| marker.boundary_message_id.as_str())
                .or(item.last_boundary_covers_message_id.as_deref())
        })
        .map(str::trim)
    ...
    if boundary == current_message_id {
        return current_message_id.to_string();
    }
    let recovered = message_store::message_store_paths(&state.data_path, conversation_id)
        .and_then(|paths| {
            ensure_ready_message_store_from_legacy_conversation(state, conversation_id, &paths)?;
            message_store::read_ready_message_store_messages_after(&paths, boundary, 1)
        })
        .ok()
        .flatten()
        .and_then(|page| page.messages.first().map(|message| message.id.clone()));
    recovered.unwrap_or_else(|| current_message_id.to_string())
}
...
async fn remote_im_group_reply_handle_timer(
    ...
    let snapshot = {
        let mut store = lock_remote_im_group_reply_state_store();
        (|| {
            let current = store.by_contact.get_mut(&action.state_key)?;
            ...
            current
                ...
                .get_or_insert_with(|| current.end_message_id.clone());
    ...
    if action.kind == RemoteImGroupReplyTimerKind::Commit {
        let Some(settlement) = snapshot.pending_settlement else {
            runtime_log_warn(format!(
    ...
    let ready = RemoteImReplyDebounceReady {
        ...
        end_message_id: snapshot
            .decision_end_message_id
            .unwrap_or(snapshot.end_message_id),
...
fn observe_remote_im_persisted_event(
    ...
    let sender_id = event
        ...
        .unwrap_or_default();
    if let Err(err) = remote_im_apply_inbound_group_energy(state, contact, sender_id, &text) {
        runtime_log_warn(format!(
            "[群聊能量] 入站结算降级，contact_id={}，message_id={}，error={}",
            contact.id, message.id, err
        ));
    }
    if let Err(err) = remote_im_recover_group_reply_delivery_marker(state, contact) {
        runtime_log_warn(format!(
            "[群聊巡检] 未确认外发恢复降级，contact_id={}，message_id={}，error={}；当前消息仍正常入队",
            contact.id, message.id, err
        ));
    }
    ...
    let mut action = None;
    {
        let mut store = lock_remote_im_group_reply_state_store();
            if let Some(current) = store.by_contact.get_mut(&key) {
                current.end_message_id = message.id.clone();
                current.event = event.clone();
                let scheduled_batch_is_frozen = current.decision_end_message_id.is_some()
                    && matches!(
                        current.phase,
                        RemoteImGroupReplyPhase::NonMentionScheduled
                            | RemoteImGroupReplyPhase::MentionScheduled
                    );
                if scheduled_batch_is_frozen {
                    current
                ...
                let generation = remote_im_group_reply_next_generation(&mut store);
                let start_message_id = remote_im_group_reply_recovery_start_message_id(
                    state,
                    &contact.id,
